...
// This function will generate a vector of size equal to 'len' that 
// comprises of random DNA sequence of fixed length 'sz'
vector<string> string_vector(int len, int sz) {
	std::vector<string> ans(len);
	for(int i = 0; i < len; i++) {
		int cur_len = sz;
		ans[i] = build_random_string(cur_len);
	}
	return ans;
}
int main() {
	...
	cin.tie(0);
	std::vector<string> ans = string_vector(100, 100);
